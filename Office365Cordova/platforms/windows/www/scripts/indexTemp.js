...
(function () {
	...
	function onDeviceReady() {
		...
		var config = {
			...
			tenant: 'flosim.net',
			clientId: "92f98787-c980-4c15-9be0-348ba4244408",
			cacheLocation: 'localStorage', // enable this for IE, as sessionStorage does not work for localhost.
		};

		authContext = new AuthenticationContext(config);
		authContext.handleWindowCallback();
		if (!authContext.getCachedUser() && !authContext.getLoginError()) {
			authContext.login(); // login if needed
		}
	};
...
function onLoad(content, type) {
	...
	if (type == "REST") {
		authContext.acquireToken(resourceID, function (error, accessToken) {
			if (content == "messages") {
